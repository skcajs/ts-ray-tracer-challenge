...
test('Multiplying by a translation matrix', () => {
    ...
    expect(p.translate(5, -3, 2)).toEqual(expected)
});
...
test('A scaling matrix applied to a point', () => {
    ...
    expect(transform.multiplyTuple(p)).toEqual(expected);
    expect(p.scale(2, 3, 4)).toEqual(expected);
});
...
test('Rotating a point around the x axis', () => {
    ...
    expect(p.rotate(0, Math.PI / 4)).toApproximatelyEqual(expected1);
    expect(fullQuarter.multiplyTuple(p)).toApproximatelyEqual(expected2);
    expect(p.rotate(0, Math.PI / 2)).toApproximatelyEqual(expected2);
});
...
test('A shearing transformation moves x in proportion to y', () => {
    ...
    expect(p.shear(1, 0, 0, 0, 0, 0)).toApproximatelyEqual(expected);
});
...
test('A shearing transformation moves x in proportion to z', () => {
    ...
    expect(transform.multiplyTuple(p)).toApproximatelyEqual(expected);
    expect(p.shear(0, 1, 0, 0, 0, 0)).toApproximatelyEqual(expected);
});
...
test('A shearing transformation moves y in proportion to x', () => {
    ...
    expect(transform.multiplyTuple(p)).toApproximatelyEqual(expected);
    expect(p.shear(0, 0, 1, 0, 0, 0)).toApproximatelyEqual(expected);
});
...
test('A shearing transformation moves y in proportion to z', () => {
    ...
    expect(transform.multiplyTuple(p)).toApproximatelyEqual(expected);
    expect(p.shear(0, 0, 0, 1, 0, 0)).toApproximatelyEqual(expected);
});
...
test('A shearing transformation moves z in proportion to x', () => {
    ...
    expect(transform.multiplyTuple(p)).toApproximatelyEqual(expected);
    expect(p.shear(0, 0, 0, 0, 1, 0)).toApproximatelyEqual(expected);
});
...
test('A shearing transformation moves z in proportion to y', () => {
    ...
    expect(transform.multiplyTuple(p)).toApproximatelyEqual(expected);
    expect(p.shear(0, 0, 0, 0, 0, 1)).toApproximatelyEqual(expected);
});
...
test('Chained rotations must be applied in reverse order compact notation', () => {
    ...
    const expected = makePoint(15, 0, 7);
    expect(p.rotate(0, Math.PI / 2).scale(5, 5, 5).translate(10, 5, 7))
        .toApproximatelyEqual(expected);
});